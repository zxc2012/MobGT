...
class Graphormer(pl.LightningModule):
    ...
    def forward(self, batched_data, perturb=None):
        ...
        # poi_pos=torch.zeros_like(poi_pos)
        # time=batched_data.time
        # time_normal=(batched_data.time_normal*48+1)/48
        time_normal=batched_data.time_normal
        # time_normal=torch.zeros_like(time_normal)
        user=batched_data.user # 0.1151
        # print(user)
        user= torch.ones_like(user) # 0.1145
        ...
        graph_embedding = self.in_degree_encoder(in_degree) + self.out_degree_encoder(out_degree)#0.1147
        node_features = (
